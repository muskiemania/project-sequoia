...
if __name__ == '__main__':
    ...
    if _args.T:
        ...
        for _each in sorted(set(_args.T)):
            _page = _bible.get_chapter(_each.lower())
            _persons = sorted([person.Person(v, _index).init() for (k,v) in _page.items()], key=lambda x: x.sort_key)
            _data.extend([[dict(person)['basic']['given'], dict(person)['basic']['middle'][0] if dict(person)['basic']['middle'] else '?', dict(person)['basic']['surname'], dict(person)['basic']['suffix'] if 'suffix' in dict(person)['basic'] else '', dict(person)['born']['on'], dict(person)['died']['on'] if 'died' in dict(person) else '----------', dict(person)['basic']['sex'], dict(person)['_id']] for person in _persons])
