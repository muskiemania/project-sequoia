...
class Buried:
    ...
    def __init__(self, person):
        ...
        try:
            ...
            self._data = dict(person).get('buried')
    ...
    def load(self, args):
        ...
        if args.on:
            try:
                __buried = datetime.datetime.fromisoformat(args.on)
                self._data['on'] = __buried.isoformat('|').split('|')[0]
            except:
                raise
    ...
    def init(self):
        ...
        if isinstance(self._data, str):
            try:
                self.__buried = datetime.datetime.fromisoformat(self._data)
            except:
                raise
            else:
                self.year = self.__buried.year

        if isinstance(self._data, dict):
            _buried = self._data.get('on')
            if _buried:
                try:
                    self.__buried = datetime.datetime.fromisoformat(_buried)
                except:
                    self.__buried = datetime.datetime(int(_buried), 1, 1)
    ...
    def __str__(self):
        ...
        _output = 'BURIED on ' + self.__buried.strftime('%b. %d, %Y')
